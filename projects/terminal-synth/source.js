...
let WAVE_RANGE = 1000; // Number.MAX_SAFE_INTEGER
...
function updateMusic() {
  ...
  if (now >= music.time) {
    ...
    music.time += musicNoteDuration(music.tempo, 1)
    position++
    ...
    if (position < music.total) {
      ...
      music.clock += musicNoteDuration(tempo, 1) / 1000.0
    }
    render()
  }
  ...
  switch (code) {
    ...
    case ' ':
      if (STATUS === STATUS_SYNTHESIZER) {
        ...
        const sum = compute(parameters)
        let buffer = SOUND_CACHE.get(sum)
        if (buffer === undefined) {
          const seconds = (parameters[ATTACK] + parameters[DECAY] + parameters[LENGTH] + parameters[RELEASE]) / 1000.0
          const samples = Math.ceil(SAMPLE_RATE * seconds)
          buffer = CONTEXT.createBuffer(1, samples, SAMPLE_RATE)
          const data = buffer.getChannelData(0)
          process(data, parameters)
          SOUND_CACHE.set(sum, buffer)
        }
        ...
        source.start(0)
        PLAY_SOUND = source
        PLAY_DATA = buffer.getChannelData(0)
        render()
      } else {
        if (MUSIC.sounds.length !== 0) {
          pauseMusic()
...
function fileWav() {
  ...
  const tracks = music.tracks
  for (let t = 0; t < tracks.length; t++) {
    length = Math.max(length, tracks[t].notes.length)
  }
  ...
  while (index < length) {
    if (index < music.tempos.length) {
      const current = music.tempos[index]
      if (current !== 0) {
        tempo = current
      }
    }
    time += musicNoteDuration(tempo, 1)
    index++
  }

  const data = new Float32Array(Math.ceil(SAMPLE_RATE * (time / 1000.0)))
  ...
  while (index < length) {
    ...
    for (let t = 0; t < tracks.length; t++) {
      ...
      if (note <= 0) continue
      let duration = 1
      for (let n = index + 1; n < notes.length; n++) {
        if (notes[n] === -1) duration++
        else break
      }
      ...
      const start = Math.round(SAMPLE_RATE * (time / 1000.0))
      for (let s = 0; s < sound.length; s++) {
        data[start + s] += sound[s]
      }
    }
    time += musicNoteDuration(tempo, 1)
    index++
  ...
  const bytesForData = data.length * 2
...
function visualize() {
  ...
  const data = PLAY_DATA

  const length = Math.min(data.length, WAVE_RANGE)
  const samplesPerPixel = Math.ceil(length / width)

  for (let w = 0; w < width; w++) {
    const p = x + w

    let s = w * samplesPerPixel
    if (s >= length) break

    let e = (w + 1) * samplesPerPixel
    ...
    const top = Math.ceil(high * half) + middle
    const bottom = Math.floor(low * half) + middle

    // console.log('LOW', low, 'HIGH', high, 'BOTTOM', bottom, 'TOP', top)

    if (bottom === top) {
      TERMINAL[WIDTH * bottom + p] = '='
    } else if (bottom + 1 === top) {
      TERMINAL[WIDTH * bottom + p] = '+'
      TERMINAL[WIDTH * top + p] = '+'
    } else {
      TERMINAL[WIDTH * bottom + p] = '%'
      for (let h = bottom + 1; h < top; h++) {
        TERMINAL[WIDTH * h + p] = '-'
      }
      TERMINAL[WIDTH * top + p] = '!'
    }
...
function process(data, parameters) {
  ...
  attack = Math.floor((attack / 1000.0) * SAMPLE_RATE)
  decay = Math.floor((decay / 1000.0) * SAMPLE_RATE)
  length = Math.floor((length / 1000.0) * SAMPLE_RATE)
  release = Math.floor((release / 1000.0) * SAMPLE_RATE)
